...
const CharacterBaseMetadata = (image: ImageFunction) => z.object({
...
export type CharacterBaseMetadata = z.infer<ReturnType<typeof CharacterBaseMetadata>>;

const CharacterDivinityMetadata = (image: ImageFunction) => CharacterBaseMetadata(image).extend({
    kind: z.literal("divinity"),
});

const CharacterMetadata = (image: ImageFunction) => z.union([
    CharacterBaseMetadata(image),
    CharacterDivinityMetadata(image),
]);
export type CharacterMetadata = z.infer<ReturnType<typeof OrganizationMetadata>>;

const LocationBaseMetadata = (image: ImageFunction) => z.object({
...
const LocationSettlementMetadata = (image: ImageFunction) => LocationBaseMetadata(image).extend({
...
const LocationMetadata = (image: ImageFunction) => z.union([
...
const HistoricMetadata = (image: ImageFunction) => z.object({
...
const OrganizationBaseMetadata = (image: ImageFunction) => z.object({
...
export type OrganizationBaseMetadata = z.infer<ReturnType<typeof OrganizationBaseMetadata>>;

const OrganizationCountryMetadata = (image: ImageFunction) => OrganizationBaseMetadata(image).extend({
  kind: z.literal("country"),
  capital: z.string().optional(),
  cultures: z.string().array().optional(),
  languages: z.string().array().optional(),
});
export type OrganizationCountryMetadata = z.infer<ReturnType<typeof OrganizationCountryMetadata>>;

const OrganizationMetadata = (image: ImageFunction) => z.union([
    OrganizationBaseMetadata(image),
    OrganizationCountryMetadata(image),
]);
export type OrganizationMetadata = z.infer<ReturnType<typeof OrganizationMetadata>>;

const CultureKind = z.enum([
    "urban",
    "rural",
    "nomadic",
    "primitive",
...
const CreatureMetadata = (image: ImageFunction) => z.object({
...
const FamilyMetadata = (image: ImageFunction) => z.object({
...
const ArticleMetadata = (image: ImageFunction) => z.union([
...
export const ArticleFrontmatter = (image: ImageFunction) => z.object({
  title: z.string(),
